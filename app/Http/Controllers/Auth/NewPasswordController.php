...
class NewPasswordController
{
    ...
    public function store(Request $request, AuditLogger $audit): RedirectResponse
    {
        ...
        $status = Password::reset($request->only('email', 'password', 'password_confirmation', 'token'), function (User $user, string $password) use ($audit): void {
            ...
            app(SessionManager::class)->revokeAll($user);
            event(new PasswordReset($user));
        });

        return $status === Password::PASSWORD_RESET ? redirect()->route('login')->with('status', 'Senha redefinida com sucesso.') : back()->withErrors(['email' => __($status)]);
    }
}
